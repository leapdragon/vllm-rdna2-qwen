...
#   ./containers/build.sh --base          first build the base: TheRock 7.14 + torch/triton/vision from
#                                         source (2–3 h on 32 cores). Needs a TheRock 7.14 install:
#                                         ROCM_DIR=/opt/rocm (default; symlinks are resolved)
...
BASE_TAG="${BASE_TAG:-therock7.14.0rc3-torch2.12-gfx1030}"
BASE_IMAGE="${BASE_IMAGE:-${REGISTRY_REPO}-base:${BASE_TAG}}"
ROCM_DIR="$(readlink -f "${ROCM_DIR:-/opt/rocm}")"
...
if [ "$do_base" = 1 ]; then
  ...
  log "base $BASE_IMAGE from TheRock at $ROCM_DIR (torch/triton/vision from source, MAX_JOBS=$MAX_JOBS)"
  docker buildx build --load --progress=plain -f "$here/Dockerfile.base" --target final \
    --build-context "rocm=$ROCM_DIR" --build-arg "MAX_JOBS=$MAX_JOBS" \
