...
| 2026-09-06 | Runtime at `eed99f623` — first image built from **`main`** (the former working branch was fast-forwarded into main and main is now the default branch) | **PASS (smoke).** Docs-only change since `8931201fc`: the serve script and entrypoint in the image are byte-identical to the serve-tested image; dry run resolves the capture sizes and the image's TunableOp rows. Published as `20260906-geed99f623` = `latest`. |
