...
Expect the RESULTS.md numbers once warm (measured from this image 2026-09-05: 65 t/s at MTP=0 over
256 tokens; 71–74 t/s at MTP=3 over 1024-token runs on 2026-09-01). TunableOp rows for the image's
own rocBLAS build ship in `tunableop/rocblas-9847aecc4bf8/`; a host build from source gets rows for
*its* rocBLAS build only after one `TUNEOP_TUNING=1` boot (the serve log says which case applies). **The first few minutes after a boot decode slower** (we saw 50–59 t/s): the 32 GB sidecar is
...
| 2026-09-05 | Runtime rebuilt at fork `1c369059b` (PLE doorbell/populate/faster lookup, tuned MoE config, QSA tiles, P2P=SYS default, TunableOp rows lookup-only) | **FAIL at first serve:** every worker died in `profile_run` with `Expected iter != ops_.end()` — the TunableOp rows tuned on the host's TheRock 7.14.0rc3 rocBLAS name solution ids this tarball's rocBLAS (same version string) does not offer for ~190 of 290 rows. The image had been unbootable since the rows shipped on 09-04 (never serve-tested). |
| 2026-09-05 | Runtime at `c41f8f4c8`: rows keyed by `sha256(librocblas.so)[:12]`, lookup disabled when none match | **PASS.** Fresh compile-cache volume, MTP=0: healthy after 16 min (cold compile); log shows the `TunableOp: no tuned rows for this rocBLAS build (9847aecc4bf8…) — lookup disabled` line, PLE doorbell + populate, tuned MoE config loaded; validate pass; **decode 65.0 / 65.3 / 65.1 t/s** (host build: 64.0). Published as `20260906-gc41f8f4c8` = `latest`. |
| 2026-09-05 | TunableOp tuning session inside that image (`-e TUNEOP_TUNING=1`, the build's rows directory bind-mounted out) | **PASS.** 27 prompt lengths (74–22,883 tokens), 392 rows per rank, every row probes clean on its own device; shipped as `tunableop/rocblas-9847aecc4bf8/`, so images from this tree run the dense GEMMs tuned again. |
| 2026-09-06 | Runtime at `db780928e` (this branch; docs-only change since `8931201fc`) | **PASS (smoke).** Serve script and entrypoint byte-identical to the serve-tested `8931201fc` image; dry run resolves the capture sizes and the image's TunableOp rows. Published as `20260906-gdb780928e` = `latest`. (`20260906-geed99f623`, built during a brief fast-forward of `main` that was reverted the same night, remains on GHCR as history.) |
| 2026-09-06 | Runtime at `8931201fc`: piecewise CUDA graphs captured for prefill batches up to 256 tokens (`CG_SIZES`, CHANGES.md §8e) | **PASS.** Fresh compile-cache volume, MTP=0, 160 W caps: cold boot 15.5 min; validate pass; in-container **short-prompt TTFT 0.24–0.27 s** (0.38–0.42 before), prefill 1101 @3.3k / 1220 @30k, **decode 64.9–65.3 t/s**; KV pool 309k tokens (353k before, the graphs' cost). Published as `20260906-g8931201fc` = `latest`. |
| 2026-09-05 | Runtime at `d487b3409` with the container-tuned rows | **PASS.** Serve log resolves `tunableop/rocblas-9847aecc4bf8/` lookup-only; cold boot 17.5 min (the changed tree invalidated the compile cache), validate pass; in-container **prefill 1125 tok/s at 3.3k, 1212 at 30k; decode 64.3–64.5 t/s** at 160 W caps. Published as `20260906-gd487b3409` = `latest`. |
...
| 2026-09-07 | Runtime at `5ea887496` (int8 shadows only, `DENSE_INT8_ONLY`, T45b + docs) | **PASS (serve).** Cold boot 16 min on 4× V620; validate PASS; prefill 1130 / 1242 tok/s @3.3k / @30k; short-prompt TTFT 0.25 s; decode 65.0–65.2 t/s (previous image 64.3–64.5); PLE lookup 1.7 ms; clean stop. Default knobs unchanged (the new mode is off unless `-e DENSE_INT8_ONLY=1`, then use `-e GPUUTIL=0.93`). Pushed as `20260907-g5ea887496` = `latest`, digest `sha256:78188228d63f`. |
| 2026-09-15 | Runtime at `1bdbbef4b` (T44b: all-reduce wedge diagnosis, fail-fast, RCCL on next boot) | **Built and published, NOT serve-tested** (maintainer's call: shipped for a reporter whose board wedges; ours never has). Smoke: build clean, incremental `_rocm_C` rebuild with the new `rdna_ar_timeout_info` op. Pushed as `20260915-g1bdbbef4b` = `latest`, digest `sha256:c6adf3dd`. On a wedge expect `rdna_ar: WEDGED -- …` then an engine stop; the next start logs `rdna_ar: disabled -- a previous run wedged` and runs on RCCL; delete `/compile-cache/rdna_ar_wedged` to retry P2P. |
