...
rebuild it from a clean host without this repository's history. Where `RESULTS.md` (T42–T46)
and `CHANGES.md` have the long form, this file has the *procedure*.
...
- **GPUs**: 5× AMD Radeon PRO V620 (Navi 21, gfx1030, 32 GB, 72 CUs, ~506 GB/s measured
  DRAM ceiling — `PROFILE-NAVI21.md`). vLLM uses four of them:
...
- **Kernel** 7.0.0-27-generic with the TP=4 stability command line (T41; see TROUBLESHOOTING.md §4):
...
- Docker 29.x. Power caps 232 W. A fan controller must be running before
...
docker run -d --name pleq-serve --entrypoint python3 \
  --device /dev/kfd --device /dev/dri --group-add "$(getent group render | cut -d: -f3)" --group-add "$(getent group video | cut -d: -f3)" \
...
| Launcher `qwen4exp-pleq/serve-pleq.sh` | this repo | edit the four absolute `$REPO/...` mounts and the `--group-add` ids (= `render`, `video` on this host) |
