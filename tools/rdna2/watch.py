...
"""llama-server-style runtime reporting for vLLM, derived from /metrics deltas.
    ...
    python tools/rdna2/watch.py            # one line per second while the server is busy
    (prefill t/s for the current burst, generation t/s, prefix-cache hit rate,
     MTP acceptance, running/waiting requests, KV usage)
...
WANT = {"vllm:prompt_tokens_total": "prompt", "vllm:generation_tokens_total": "gen",
        ...
        # MTP speculative decoding: accepted / drafted tokens per tick
...
while True:
    ...
    if True:
        ...
        da = cur.get("acc", 0.0) - prev.get("acc", 0.0)
        dd = cur.get("draft", 0.0) - prev.get("draft", 0.0)
        if dd > 0: bits.append(f"mtp {da/dd*100:4.0f}% accepted")
